...
# DhG_PrintFamily() - prints the family (parents, siblings, children) of the specified person
sub DhG_PrintFamily
{
	my ($id) = @_;
	my ($index, $person);
	my ($father, $fid, $mother, $mid);
	my %siblings;
	my ($sid, $sname, $prefix, $suffix);		# Sibling
	my %children;
	my ($cid, $cname);		# Child
	my ($spouse, $spouseuniq, $spouseinfo, $xxx);

	$name = $DhG_Name[$id];
	$person = DhG_GetPersonInfoLine($id);
	...
	if ( defined $father )
	{
		...
		if ( defined $fid )
		{
			$father = DhG_GetPersonInfoLine($fid);
		}
	}
	else
	{
	...
	if ( defined $mother )
	{
		...
		if ( defined $mid )
		{
			$mother = DhG_GetPersonInfoLine($mid);
		}
	}
	else
	{
	...
	print "\n";
	print "Immediate relatives of $person\n";
	print "\n";
	...
	foreach $sid ( 1 .. $#DhG_Name )
	{
		if ( defined $DhG_Name[$sid] )
		{
			if ( (( defined $DhG_Father_Id[$sid] && defined $fid && $DhG_Father_Id[$sid] == $fid ) ||
				  ( defined $DhG_Mother_Id[$sid] && defined $mid && $DhG_Mother_Id[$sid] == $mid )) )
			{
				$xxx = $DhG_Birth_Date[$sid];
				if ( !defined $xxx )
				{
					$xxx = "?";
				}

				while ( defined $siblings{$xxx} )
				{
					$xxx .= _;
				}
	...
	foreach $xxx (sort keys %siblings)
	{
		$sid = $siblings{$xxx};
		$sname = DhG_GetPersonInfoLine($sid);
		...
		if ( defined $DhG_Father_Id[$sid] && defined $fid && $DhG_Father_Id[$sid] != $fid )
		{
			$suffix = "  (Father: $DhG_Father_Name[$sid]";
			if ( defined $DhG_Father_Id[$sid] )
			{
				$suffix .= "($DhG_Father_Id[$sid])";
			}
			$suffix .= ")";
		}
		elsif ( defined $DhG_Mother_Id[$sid] && defined $mid && $DhG_Mother_Id[$sid] != $mid )
		{
			$suffix = "  (Mother: $DhG_Mother_Name[$sid]";
			if ( defined $DhG_Mother_Id[$sid] )
			{
				$suffix .= "($DhG_Mother_Id[$sid])";
			}
			$suffix .= ")";
		}
		else
		...
		print "$prefix $sname$suffix\n";
	}

	foreach $cid ( 1 .. $#DhG_Name )
	{
		if ( defined $DhG_Name[$cid] )
		{
			if ( ( defined $DhG_Father_Id[$cid] && $DhG_Father_Id[$cid] == $id ) ||
				 ( defined $DhG_Mother_Id[$cid] && $DhG_Mother_Id[$cid] == $id ) )
			{
				$xxx = $DhG_Birth_Date[$cid];
				if ( !defined $xxx )
				{
					$xxx = "?";
				}

				while ( defined $children{$xxx} )
				{
					$xxx .= _;
				}
	...
	foreach $xxx (sort keys %children)
	{
		$cid = $children{$xxx};
		$cname = DhG_GetPersonInfoLine($cid);

		if ( $DhG_Father_Name[$cid] eq $name )
		{
			if ( $DhG_Mother_Name[$cid] ne $spouse )
			{
				$spouse = $DhG_Mother_Name[$cid];
				$spouseuniq = $DhG_Mother_Uniq[$cid];
				$spouseinfo = DhG_GetPersonInfoLine($spouse, $spouseuniq);
				print "\n";
				print "Children with $spouseinfo:\n";
			}
		}
		else
		{
			if ( $DhG_Father_Name[$cid] ne $spouse )
			{
				$spouse = $DhG_Father_Name[$cid];
				$spouseuniq = $DhG_Father_Uniq[$cid];
				$spouseinfo = DhG_GetPersonInfoLine($spouse, $spouseuniq);
				print "\n";
				print "Children with $spouseinfo:\n";
			}
